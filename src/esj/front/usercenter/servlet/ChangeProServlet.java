 ...
 * @作者 朱志玉
 * @项目名称 XYESJ
 * @创建时间 2016-7-18 上午8:44:59
 * @最后修改时间 2016-7-18 上午8:44:59
 * @描述：
 * @version 1.0
 */
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
	  ...
	  //request.setCharacterEncoding("UTF-8");
        ...
        if(upload.isMultipartContent(request)){
			...
			PProductsBeanService pService = new PProductsBeanServiceimpl();
			int count =  pService.updateProduct(productsBean);//保存商品信息
			try {
				if(count>0){
					response.sendRedirect(request.getContextPath()+"/person_center/SoldProServlet?op=1");
