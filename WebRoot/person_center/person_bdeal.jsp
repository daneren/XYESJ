				  ...
				  <thead>
				   ...
				   <c:forEach var="ars" items="${uDeal}" varStatus="status">
					   <tr>
						   ...
						   <td>${ars.bdate}</td>
						   <td id="td${ars.reservId}"><a href="javascript:if(confirm('您点击了交易成功，确定保存该信息吗?'))location='${pageContext.request.contextPath }/person_center/BDealing?op=2&id=${ars.reservId},${ars.proId}'" class="cuss">交易成功</a>|<a href="javascript:if(confirm('您点击了交易失败，确定保存该信息吗?'))location='${pageContext.request.contextPath }/person_center/BDealing?op=3&id=${ars.reservId},${ars.proId}'" class="fail">交易失败</a></td>
					   </tr>
				   </c:forEach>
				   </tbody>
				  </table>
