...
List<UsersBean> list =service.findAllUsers();
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 用户中心 <span class="c-gray en">&gt;</span> 用户信息管理 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="pd-20"><%--
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
     <span class="l">
        <span class="r">共有数据：<strong>22</strong> 条</span>
    </div>
    --%>
    <div class="mt-20">
        <table class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            <td><a class="maincolor" href="javascript:;" onclick="member_show('个人信息','user_infom.jsp?id=${ usersBean.userId}','500','500')" >${ usersBean.userName}</a></td>
            <td>${ usersBean.rigistDay}</td>
            <td>${ usersBean.sex}</td>
            <td>${ usersBean.birthDay}</td>
            <td>${ usersBean.school}</td>
            <td>${ usersBean.department}</td>
            <td>${ usersBean.major}</td>
            <td>${ usersBean.entranceDay}</td>
            <td>${ usersBean.empirical}</td>
            <td>${ usersBean.creditPoints}</td>
            <td>${ usersBean.iconAddr}</td>
            <td>${ usersBean.email}</td>
            <td>${ usersBean.remark}</td>
            <c:if test="${ usersBean.state =='1'}">
	            <td class="td-status"><span class="label label-success radius">已启用</span></td>
	            <td class="td-manage">
	            <a style="text-decoration:none" onClick="member_stop(this,'${ usersBean.userId}')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a>
	            ...
	            <td class="td-manage">
	            <a style="text-decoration:none" onClick="member_start(this,'${ usersBean.userId}')" href="javascript:;" title="启用"><i class="Hui-iconfont">&#xe6e1</i></a>
...
return ajax;
    ...
    function member_stop(obj,id){
        layer.confirm('确认要停用吗？',function(index){
        	var ajax = createAJAX();
		    var method = "GET";
		    var url = "${pageContext.request.contextPath}/servlet/admin_userschangestateServlet?id="+id+"&state="+0+"&time="+new Date().getTime();
    ...
    function member_start(obj,id){
        ...
        var ajax = createAJAX();
		    var method = "GET";
		    var url = "${pageContext.request.contextPath}/servlet/admin_userschangestateServlet?id="+id+"&state="+1+"&time="+new Date().getTime();
		    ajax.open(method,url);
		    //alert(url);
		    ajax.send(null);
		    ajax.onreadystatechange = function(){
