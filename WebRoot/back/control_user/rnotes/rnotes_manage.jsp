<%@page import="esj.publicdoc.bean.RnotesBean"%>
<%@page import="esj.publicdoc.service.RnotesBeanService"%>
<%@page import="esj.publicdoc.service.impl.RnotesBeanServiceimpl"%>
<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>
<%@taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%
RnotesBeanService service=new RnotesBeanServiceimpl();
List<RnotesBean> list =service.findAllRnotes();
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>用户中心 <span class="c-gray en">&gt;</span> 用户管理<span class="c-gray en">&gt;</span>回复管理 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="pd-20">
    ...
    --%><div class="mt-20">
        <table class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            <tbody>
            <c:forEach var="rnotesBean" items="${list }" varStatus="status">
	        	<tr class="text-c">
	            <td><input type="checkbox" value="1" name=""></td>
	            <td>${ rnotesBean.replyId}</td>
	            <td> ${ rnotesBean.userId}</td>
	            <td>${ rnotesBean.leaveId}</td>
	            <td>${ rnotesBean.details}</td>
	            <td>  </td>
	            <td>${ rnotesBean.bdate}</td>
	            <td>${ rnotesBean.ip}</td>
	            <td>${ rnotesBean.ipAddr}</td>
	            <c:if test="${ rnotesBean.state ==1}">
	            <td class="td-status"><span class="label label-success radius">已启用</span></td>
	            <td class="td-manage">
	            <a style="text-decoration:none" onClick="member_stop(this,'${rnotesBean.replyId}')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a>
	            </td>
	           	</c:if>
	           	<c:if test="${ rnotesBean.state ==0}">
	            <td class="td-status"><span class="label label-defaunt radius">已停用</span></td>
	            <td class="td-manage">
	            <a style="text-decoration:none" onClick="member_start(this,'${rnotesBean.replyId}')" href="javascript:;" title="启用"><i class="Hui-iconfont">&#xe6e1</i></a> 
	            </td>
	           	</c:if> 
	      		</tr>
	      		</tr>
...
return ajax;
    ...
    function member_stop(obj,id){
        layer.confirm('确认要停用吗？',function(index){
        	var ajax = createAJAX();
		    var method = "GET";
		    var url = "${pageContext.request.contextPath}/servlet/admin_rnoteschangestateServlet?id="+id+"&state="+0+"&time="+new Date().getTime();
    ...
    function member_start(obj,id){
        ...
        var ajax = createAJAX();
		    var method = "GET";
		    var url = "${pageContext.request.contextPath}/servlet/admin_rnoteschangestateServlet?id="+id+"&state="+1+"&time="+new Date().getTime();
		    ajax.open(method,url);
		    ajax.send(null);
		    ajax.onreadystatechange = function(){
		        if(ajax.readyState == 4){    
